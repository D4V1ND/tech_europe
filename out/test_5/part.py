...
# --- multi-body assembly ---
result = None
add0 = cq.Workplane('XY').box(116.0, 97.0, 3.0, centered=(False, False, False)).translate((0.0, 8.0, 97.0))
result = add0 if result is None else result.union(add0)
add1 = cq.Workplane('XY').box(35.0, 3.0, 100.0, centered=(False, False, False)).translate((0.0, 0.0, 0.0))
result = add1 if result is None else result.union(add1)
add2 = cq.Workplane('XY').box(35.0, 3.0, 100.0, centered=(False, False, False)).translate((81.0, 0.0, 0.0))
...
add4 = cq.Workplane('YZ').workplane(offset=0.0).polyline([(0.0, 0.0), (0.0, 100.0), (105.0, 100.0), (82.0, 60.0), (58.0, 25.0), (30.0, 0.0)]).close().extrude(3.0)
result = add4 if result is None else result.union(add4)
add5 = cq.Workplane('YZ').workplane(offset=113.0).polyline([(0.0, 0.0), (0.0, 100.0), (105.0, 100.0), (82.0, 60.0), (58.0, 25.0), (30.0, 0.0)]).close().extrude(3.0)
