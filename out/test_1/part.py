...
result = result.cut(cq.Workplane('XY').workplane(offset=thickness + 0.01).pushPoints([(hole1_x, hole1_y)]).circle(hole1_dia / 2).extrude(-(hole1_depth + 0.01)))
...
result = result.cut(cq.Workplane('XY').workplane(offset=thickness + 0.01).pushPoints([(hole3_x, hole3_y)]).circle(hole3_dia / 2).extrude(-(hole3_depth + 0.01)))
...
result = result.cut(cq.Workplane('XY').workplane(offset=thickness + 0.01).pushPoints([(hole5_x, hole5_y)]).circle(hole5_dia / 2).extrude(-(hole5_depth + 0.01)))
result = result.cut(cq.Workplane('XY').workplane(offset=thickness + 0.01).pushPoints([(hole6_x, hole6_y)]).circle(hole6_dia / 2).extrude(-(thickness + 0.02)))
...
result = result.cut(cq.Workplane('XY').workplane(offset=thickness + 0.01).pushPoints([(hole8_x, hole8_y)]).circle(hole8_dia / 2).extrude(-(thickness + 0.02)))
